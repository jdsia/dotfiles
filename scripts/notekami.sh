  ...
  case $selected in
    ...
    search)
      ...
      if [[ "$TERM_PROGRAM" = tmux ]]; then
        tmux new-window -n "search-notes" "${EDITOR:-vim} +$file_num $file_name"
      else
        "${EDITOR:-vim}" +"$file_num" "$file_name"
      fi
      ;;
    create)
      ...
      if [ "$TERM_PROGRAM" = tmux ]; then
        tmux new-window -n "create-cheatsheet" "${EDITOR:-vim} $mydir/$selected"
      else
        "${EDITOR:-vim}" "$mydir"/"$selected"
      fi
      ;;
    copy)
    ...
    edit)
      if [ "$TERM_PROGRAM" = tmux ]; then
        ...
        echo "$selected" | while read -r line; do
          ...
          tmux new-window -n "${filename}-pill" "${EDITOR:-vim} $line"
        done
      else
        selected="$(find "$mydir"/ -type f | sort | fzf -d / --with-nth -2.. \
          --preview 'bat --color=always -pp {}' \
    ...
    snippet)
      selected=$(grep -nrH '^####; ' "$mycheatdir"/* | fzf -d ' ' --with-nth 2.. \
        --prompt="notekami ($selected) >>> copy to clipboard: " \
        --bind 'ctrl-f:execute:${EDITOR:-vim} $(echo {} | cut -d: -f1) >/dev/tty' \
        --bind 'ctrl-o:execute:${EDITOR:-vim} +$(echo {} | cut -d: -f2) $(echo {} | cut -d: -f1) >/dev/tty' \
